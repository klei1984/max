...
- Ditch the entire Link layer (or Data Link and Physical layers) together with Null Modem, Dial-Up Modem and IPX LAN play and replace all of these with a single TCP/IP or UDP/IP based alternative design. Which transport layer protocol to use could depend on the way broadcast messages are used by IPX LAN play and how acknowledgement of packets works.
...
- *Target Node (2 bytes)*: Packet Type specific meaning.
...
*target node* Address of the node that shall process the packet.
...
IPX unicast message sent by each player to each other after the host sent *[Packet 17](#packet_ref17)*. The *target_node* is set to node address 0 to 3 representing the team slots (Red, Green, Blue and Gray).
...
IPX broadcast message sent by client that wants to join a game. The packet is only sent by a client if the host and its node address is not yet known. E.g. if the client did not receive any *[Packet 44](#packet_ref44)* messages yet. The *target_node* in the header is set to 0x0000. As the host is not known yet the client's node address is also not known yet so the *source_node* field is also set to 0x0000.
...
*source_node* Node address of the client.
*data* See [Packet 31](#packet_ref31).

<a name="packet_ref29"></a>
### Packet 29

IPX broadcast message sent by client to host indicating that client wants to join the game that host is setting up. The *target_node* is the host. The host is identified via its *[Packet 44](#packet_ref44)* message contents.
...
*address* On MS-DOS systems this is the real IPX address of the network interface. In DOSBox the first 4 bytes is the IPv4 address, the last two bytes are the UDP port.
...
*node [4]* Node address of each known player. Note that the client does not get its new node address from the host this way. The packet's *target_node* field is used for that purpose. Normally the host does not yet know which team slot will be requested by the newly registered client via *[Packet 35](#packet_ref35)*.
...
IPX broadcast message sent by a player during game setup phase to denounce itself. The *target_node* field is set to the player's node.
...
*host_node* TODO: Seems to be always 0. The value is read from the network game menu manager object. The receiver does not use the field for anything.

*data* The packet size is always 6 bytes, but the last 4 bytes (this field) are not set. Unset random buffer content is sent by the host.

<a name="packet_ref32"></a>
### Packet 32

IPX broadcast message sent by the host to start the configured network game. The *target_node* is set to the host. As response to this message each client sends *[Packet 05](#packet_ref05)* as response to the host.
...
The *target_node* is always the host, but all peers registered for the given host's game receives the message.
...
IPX unicast message sent by the host to the client to acknowledge *[Packet 05](#packet_ref05)* and to announce all players' IPX local address. The *target_node* field is set to the host.
...
IPX broadcast message sent by a player that changed their name. The *target_node* is set to the player's own node that changed the name and sent the packet.
...
IPX broadcast message sent by host when map is changed, a previous network game is loaded, a multiplaer scenario is selected or game options like starting credit is changed. The *target_node* is set to the host.
...
Broadcast message sent by host every 3 seconds or so during game setup phase to announce itself.
